      ...
      <div class="col-lg-3 mx-5">
          <div class="mt-4">
            <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
              <button type="button" id="sort-asc" class="btn btn-outline-secondary"><i class="fa-solid fa-arrow-up-short-wide"></i></button>
              <button type="button" id="sort-desc"  class="btn btn-outline-secondary"><i class="fa-solid fa-arrow-down-short-wide"></i></button>
              <select id="sort-field" class="btn btn-outline-secondary dropdown-toggle" >
                    ...
                    <option value="uploadStatus">狀態</option>
              </select>
            </div>
          </div>
        </div>
...
<script>
   const sortAscButton = document.getElementById('sort-asc');
    ...
    let sortField = sortFieldSelect.value; // 默认排序字段
    ...
    function loadArticles() {
        ...
        const url = `article.php?page=${page}&keyword=${keyword}&sortField=${sortField}&sortOrder=${sortOrder}`;
        
        // 使用 fetch 或者 window.location 来重新加载页面并传递排序信息
  ...
  viewButtons.forEach(button => {
    ...
    const deleteOne = e=>{
        ...
        delModal.show()
        document.querySelector('#exampleModalLabel2').innerHTML=`是否要刪除編號為${articleid}，標填為${title}的文章`
